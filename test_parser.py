...
class TestAxisCorporateStatement2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.pdf_path = r"C:\Users\rtrpr\.gemini\antigravity\brain\798c510d-3f38-4457-8c71-0896ee2415ef\.user_uploaded\media__1785418465609.pdf"
        cls.metadata, cls.transactions = parse_pdf(cls.pdf_path)
    ...
    def test_metadata_extraction(self):
        ...
        self.assertEqual(self.metadata.get("customer_id"), "903901942")
        self.assertEqual(self.metadata.get("account_type"), "CA - CURRENT A/C FOR ESCROW ARRANGEMENTS")
        self.assertEqual(self.metadata.get("holder_name"), "MS. VENKATA BEVERAGES PRIVATE LIMITED")

    def test_transaction_count(self):
        self.assertEqual(len(self.transactions), 0)

class TestAxisCorporateStatement3(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.pdf_path = r"C:\Users\rtrpr\.gemini\antigravity\brain\798c510d-3f38-4457-8c71-0896ee2415ef\.user_uploaded\media__1785418465634.pdf"
        cls.metadata, cls.transactions = parse_pdf(cls.pdf_path)
    ...
    def test_transaction_count(self):
        self.assertEqual(len(self.transactions), 7)

    def test_first_and_last_transactions(self):
        first_tx = self.transactions[0]
        self.assertEqual(first_tx["txn_date"], "08/04/2022")
        self.assertEqual(first_tx["credit"], "3848000.00")
        self.assertEqual(first_tx["balance"], "4270622.00")
...
class TestSBIStatement2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.pdf_path = r"C:\Users\rtrpr\.gemini\antigravity\brain\798c510d-3f38-4457-8c71-0896ee2415ef\.user_uploaded\media_1785927315363.pdf"
        cls.metadata, cls.transactions = parse_pdf(cls.pdf_path)

    def test_metadata_extraction(self):
        self.assertIsNotNone(self.metadata)
        self.assertEqual(self.metadata.get("account_number"), "31617054048")
        self.assertEqual(self.metadata.get("customer_id"), "85969391535")
        self.assertEqual(self.metadata.get("account_type"), "REGULAR SB CHQ-INDIVIDUALS")
        self.assertEqual(self.metadata.get("statement_period"), "01-04-2025 to 31-03-2026")
        self.assertEqual(self.metadata.get("holder_name"), "Mr. KESAVAN RAMESH")

    def test_transaction_count(self):
        self.assertEqual(len(self.transactions), 719)

    def test_first_and_last_transactions(self):
        first_tx = self.transactions[0]
        self.assertEqual(first_tx["txn_date"], "01-04-2025")
        self.assertEqual(first_tx["particulars"], "WDL TFR UPI/DR/509124997071/KARTHIC")
        self.assertEqual(first_tx["debit"], "60.00")
        self.assertEqual(first_tx["balance"], "7,149.69")
        
        last_tx = self.transactions[-1]
        self.assertEqual(last_tx["txn_date"], "31-03-2026")
        self.assertEqual(last_tx["particulars"], "WDL TFR UPI/DR/609026543756/ALANKAR")
...
class TestCityUnionBank(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.pdf_path = r"C:\Users\rtrpr\.gemini\antigravity\brain\0cce1afd-fba4-4b3e-9fb3-efe493272c56\.user_uploaded\media_1786164836766.pdf"
        cls.metadata, cls.transactions = parse_pdf(cls.pdf_path)

    def test_metadata_extraction(self):
        self.assertIsNotNone(self.metadata)
        self.assertEqual(self.metadata.get("account_number"), "510909010117163")
        self.assertEqual(self.metadata.get("customer_id"), "5829224")
        self.assertTrue("Current Account" in self.metadata.get("account_type", ""))
        self.assertEqual(self.metadata.get("statement_period"), "01-APR-2025 to 31-MAR-2026")
        self.assertEqual(self.metadata.get("holder_name"), "VIJAY CATERING SERVICES")

    def test_transaction_count(self):
        self.assertEqual(len(self.transactions), 4334)

    def test_first_and_last_transactions(self):
        first_tx = self.transactions[0]
        self.assertEqual(first_tx["txn_date"], "01-APR-2025")
        self.assertEqual(first_tx["credit"], "11,400.00")
        self.assertEqual(first_tx["balance"], "1,08,365.30")

        last_tx = self.transactions[-1]
        self.assertEqual(last_tx["txn_date"], "31-MAR-2026")
        self.assertEqual(last_tx["credit"], "13,678.00")
        self.assertEqual(last_tx["balance"], "3,30,454.75")

class TestAxisBankFormat2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.pdf_path = r"C:\Users\rtrpr\.gemini\antigravity\brain\0cce1afd-fba4-4b3e-9fb3-efe493272c56\.user_uploaded\media_1786164836682.pdf"
        cls.metadata, cls.transactions = parse_pdf(cls.pdf_path)

    def test_metadata_extraction(self):
        self.assertIsNotNone(self.metadata)
        self.assertEqual(self.metadata.get("account_number"), "919010027476343")
        self.assertEqual(self.metadata.get("customer_id"), "889816958")
        self.assertEqual(self.metadata.get("account_type"), "SAVINGS-EASYACCESS")
        self.assertEqual(self.metadata.get("statement_period"), "01-04-2025 to 31-03-2026")
        self.assertEqual(self.metadata.get("holder_name"), "VIJAY BABU")

    def test_transaction_count(self):
        self.assertEqual(len(self.transactions), 2022)

    def test_first_and_last_transactions(self):
        first_tx = self.transactions[0]
        self.assertEqual(first_tx["txn_date"], "01-04-2025")
        self.assertEqual(first_tx["credit"], "575.00")
        self.assertEqual(first_tx["balance"], "21078.10")

        last_tx = self.transactions[-1]
        self.assertEqual(last_tx["txn_date"], "31-03-2026")
        self.assertEqual(last_tx["credit"], "2627.00")
        self.assertEqual(last_tx["balance"], "307652.72")

class TestIDBIBankStatement(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.pdf_path = r"C:\Users\rtrpr\.gemini\antigravity\brain\0cce1afd-fba4-4b3e-9fb3-efe493272c56\.user_uploaded\media_1786364063846.pdf"
        cls.metadata, cls.transactions = parse_pdf(cls.pdf_path)

    def test_metadata_extraction(self):
        self.assertIsNotNone(self.metadata)
        self.assertEqual(self.metadata.get("account_number"), "0194653800000134")
        self.assertEqual(self.metadata.get("account_type"), "Current Account")
        self.assertEqual(self.metadata.get("statement_period"), "01-04-2025 to 08-06-2026")
        self.assertEqual(self.metadata.get("holder_name"), "HOLY ASTERIX APPARELS PRIVATE LIMITED")

    def test_transaction_count(self):
        self.assertEqual(len(self.transactions), 40)

    def test_first_and_last_transactions(self):
        first_tx = self.transactions[0]
        self.assertEqual(first_tx["txn_date"], "01-04-2025")
        self.assertEqual(first_tx["debit"], "19,081.08")
        self.assertEqual(first_tx["balance"], "20,10,878.58Dr")

        last_tx = self.transactions[-1]
        self.assertEqual(last_tx["txn_date"], "06-06-2026")
        self.assertEqual(last_tx["credit"], "18,000.00")
        self.assertEqual(last_tx["balance"], "19,52,452.09Dr")

class TestHDFCFormat2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.pdf_path = r"C:\Users\rtrpr\.gemini\antigravity\brain\0cce1afd-fba4-4b3e-9fb3-efe493272c56\.user_uploaded\media_1787657557976.pdf"
        cls.metadata, cls.transactions = parse_pdf(cls.pdf_path)

    def test_metadata_extraction(self):
        self.assertIsNotNone(self.metadata)
        self.assertEqual(self.metadata.get("account_number"), "50200048174115")
        self.assertEqual(self.metadata.get("customer_id"), "138546201")
        self.assertEqual(self.metadata.get("account_type"), "BIZULTRAPLUSACCOUNT(1483)")
        self.assertEqual(self.metadata.get("statement_period"), "01/04/2025 to 31/03/2026")
    ...
    def test_first_and_last_transactions(self):
        ...
        self.assertEqual(first_tx["credit"], "4,800.00")
        self.assertEqual(first_tx["balance"], "281,514.11")

        last_tx = self.transactions[-1]
        self.assertEqual(last_tx["txn_date"], "31/03/26")
        self.assertEqual(last_tx["debit"], "49,891.00")
        self.assertEqual(last_tx["balance"], "286,486.07")

class TestICICIFormat2(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.pdf_path = r"C:\Users\rtrpr\.gemini\antigravity\brain\0cce1afd-fba4-4b3e-9fb3-efe493272c56\.user_uploaded\media_1787738108992.pdf"
        cls.metadata, cls.transactions = parse_pdf(cls.pdf_path)

    def test_metadata_extraction(self):
        self.assertIsNotNone(self.metadata)
        self.assertEqual(self.metadata.get("account_number"), "189805001537")
        self.assertEqual(self.metadata.get("customer_id"), "568520778")
        self.assertEqual(self.metadata.get("account_type"), "Current")
        self.assertEqual(self.metadata.get("statement_period"), "01-04-2025 to 31-03-2026")
    ...
    def test_first_and_last_transactions(self):
        first_tx = self.transactions[0]
        self.assertEqual(first_tx["txn_date"], "05-04-2025")
        self.assertEqual(first_tx["credit"], "13,000.00")
        self.assertEqual(first_tx["balance"], "13,000.01 Cr")

        last_tx = self.transactions[-1]
        self.assertEqual(last_tx["txn_date"], "19-01-2026")
